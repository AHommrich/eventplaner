...
test.describe('owner event settings', () => {
    ...
    test('persists a primary color change', async ({ page }) => {
        ...
        await page.waitForResponse(
            (res) => res.url().includes('/event/settings') && res.request().method() === 'POST' && res.status() < 400,
        );
