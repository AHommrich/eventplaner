...
The role foundation is **half-built already**. This project is a *refinement*, not a greenfield.
...
**Key gap:** pivot members currently reach the *entire* `has_event` route group, which
...
- **Co-ownership (first-class):** an event may have **multiple equal owners** — required for the
  primary use case (a wedding couple: André *and* Tabea are both full owners, not owner + helper).
...
the event** — the primary `event.user_id` *and* any co-owner (pivot role `owner`); all owners are
...
**Hard security boundary:** an Owner/Event-Admin can grant *per-event* roles only. **Granting the
...
- **Co-ownership model:** ownership = `event.user_id` (the *primary* owner: creator/anchor, kept for
...
- **Last-owner protection — scope precisely (Codex #2, resolved):** it guards against **losing the
  last owner by accident**, i.e. it blocks *demotion / removal / transfer-away* that would leave the
  ...
  *not* subject to last-owner protection; only `manageAccess`-demotion and `transferOwnership` are.
...
- **Ownership transfer must be one atomic transaction (Codex #2, resolved):** swapping
  ...
  4. Remove the now-redundant pivot `owner` row of the *new* primary (they're covered by
...
- `photos/projector-album` (`web.php:58`), `photos/projector-name-mode` (`:59`),
  ...
  *immediately locks out the running public projector screen* — that is event configuration, not
...
- **`/requests` page — split visibility by section (round-6b #4b, decided: managers do NOT see photo
  ...
  guest complaint to someone who cannot act on it. *(Flip to "manager sees read-only" only if André
  later wants managers aware of complaints.)*
...
| `EventAccessController` index/invite/remove (`:21,:43,:70`) | ⚠️ `event.user_id === user->id` → primary owner OR superadmin only — even today's pivot co-organizers blocked | **P1** move to `manageAccess` policy (pivot `owner` + `event_admin` pass). *Previous "keep" was WRONG (Codex #1). No P0 interim — loosening early would let any member invite/remove (Codex round-4 #1); it stays owner/superadmin until P1.* | add |
...
- `notes` table: `id, event_id (FK, cascade), author_user_id (FK, **nullable, nullOnDelete**),
  ...
  is_done (bool, todos only), done_at (nullable), timestamps`.
  - **FK nullability is mandatory (Codex #3, verified):** user deletion in this project is a **hard
  ...
  - `assignee_user_id = null` → personal (author's own).
  - `assignee_user_id = <manager>` → owner-assigned task (only owner/admin may create these).
    The assignee sees these labelled as **"created by the event organizer for you"**
    (de: "Vom Veranstalter für dich erstellt"). Assignee sees their *own* assigned items;
...
- Create an **assigned** note (assignee set) — dedicated ability, unambiguous (Codex #4, resolved):
  ...
  - **Assignee = exclusively an *active* `event_manager` of that event** (validated against
...
- **The middleware re-checks the full authorization on EVERY request (round-5 #5), because a Sanctum
  bearer outlives the login check.** Login (§6.1) verifies email + approval *once*; a token issued
...
- **Decision: managers do NOT reuse the guest QR path.** A manager processes *all* guests' personal
...
- **OAuth users have no password** (Google login via `SocialLoginController`). For them, plain
  ...
  This is why the pairing QR is the *recommended* onboarding, not just a nicety.
- **Actor-type isolation (own finding):** both `Guest` and `User` are `HasApiTokens` under the same
  `sanctum` guard, so `$request->user()` may resolve to *either*. `EnsureGuestHasAppAccess` assumes a
...
- **Recommended onboarding — one-time pairing QR (not the guest QR):** owner/admin generates
  ...
  so it is a *pairing bootstrap*, not a standing bearer credential like the guest QR. Gives guest-like
...
- **Pairing-token hygiene (mandatory — Codex #8):**
  ...
  - **Per-device revocation needs a token reference (Codex round-4 #6):** to revoke a device the
    system must know *which* Sanctum token to delete. `user_id` + `redeemed_at` + `device_label` are
  ...
  Plain email/password login remains as a fallback.
- The app needs a mode switch: "I'm a guest" (QR) vs "I'm an organizer" (email/password + optional
  bootstrap QR).
...
- **Photo-badge semantics with co-owners (Codex #7, resolved — snapshot the role at upload):**
  the current logic (`PhotoController.php:54-57`) resolves the badge *dynamically* as
...
- **The snapshot union MUST match `roleOn()` exactly (Codex round-4 #5):** `roleOn()` can return
  ...
  *label* a superadmin upload shows (recommendation: render superadmin as the plain owner/organizer
...
- **"Open app, else store" from the QR landing page.** Browsers cannot *detect* an installed app
...
- **Notes retention — full specification, not just "soft deletes" (Codex round-4 #8).** Soft-delete
  ...
  - **Hard-purge trigger:** soft-deleted notes are permanently removed after N days
    (`RETENTION_NOTES_DAYS`, new key in `config/retention.php`); *all* notes of an event are purged
...
0. **P0 — Cleanup / multi-tenancy hardening (do FIRST, before any role code).**
   *Decided with André 2026-07-16: clean the dirty controllers before layering a policy on top —
   ...
   on its own (pure cross-event 403 Pest tests, no new roles involved).*
   - **P0.1 — `categories` + `food_specials` become per-event** (Codex #5; round-4 #2: a naive
     ...
     guests of *different* events via `guests.category_id` and the `guest_food_special` pivot).
     - **Data model — DECIDED (André, 2026-07-16): the `PhotoGameTaskCatalog` delta pattern.**
       ...
       *(Clone-and-repoint with `NOT NULL` was the considered alternative — rejected: duplicates the
       catalog per event and forces every creation path to seed. Not used.)*
   ...
   - **P0.3 — `GuestController::store`/`update` reference validation** (Codex round-4 #2, verified).
     ...
     - **`food_specials` must ALSO accept the global templates** under the delta model (round-6 #1 —
       ...
       Private rows of *other* events stay rejected. **`categories` use the strict form** (no
       `orWhereNull` — they have no templates, `event_id` is NOT NULL, see P0.1).
     - Add Pest tests: a foreign group_id is rejected; a foreign event's private food-special is
       rejected; a global template (`event_id = null`) is **accepted**.
     *(This replaces the previously-planned P0.3 EventAccessController interim — see P1.)*
   ...
   - **P0.5 — User-deletion is a data-loss bug today (Codex round-4 #3, verified — SEVERE).**
     ...
     is an equal owner. This is broken *before* the role feature and must be fixed in P0:
   ...
   - **P0 commit checkpoints — each is one green-CI commit, safe to push, in this order.** Every
     ...
     1. **Checkpoint A — P0.5** (user-deletion / FK `RESTRICT`). Self-contained: migration + guard +
        Pest. No dependency on the others. *(Do first — highest data-loss risk.)*
     ...
     change — the migration + its dependent controller edits are always in the *same* commit.
1. **P1 — Authorization backbone (web-only, invisible to users).**
   ...
   (§8.1). *Highest value, lowest risk. Ships the actual "manager can't administer" behaviour.*
...
4. **P4 — Management API + auth.** `HasApiTokens` on `User` + verified/approved/throttle gating,
   ...
   `/api/management/*`. *The three "Codex blockers" live here; none block P1.*
...
*today*. The `role` backfill to `event_manager` **silently downgrades them** (loses deep settings /
...
- **OAuth users have no password** → pairing QR is their only login path, which is *why* it's the
...
folded in. Notable: two were *worse* than stated.
...
2. **`categories`/`food_specials` need a *real* multi-event migration** (§11 P0.1) — verified
...
2. **New events need a food-special default after the P0.1 split** (§11 P0.1) — verified
   `food_specials` is a globally *seeded* i18n catalog and both creation paths
   ...
   event-local; manager writes always event-scoped. Solves defaults *and* isolation without cloning.
...
1. **P0.3 validation must accept global templates** (§11 P0.3) — under the delta model a food-special
   ...
   *private* rows still rejected. Pest tests updated (template accepted, foreign-private rejected).
