...
- 2026-07-08 — UX round two follow-ups: (a) Event owner relation is `owner()`, not `user()` — the report-notification mail was silently skipping the send. (b) Cascade on `photo_reports.photo_id` was destroying the moderation trail on photo-delete; new migration `2026_07_08_130000_relax_photo_reports_photo_id_fk` nullifies `photo_id` on photo delete instead so the resolved report survives as audit history. (c) Test helper `makePhotoReport` now uses `firstOrCreate` for the gallery album so multi-report scenarios do not trip the unique constraint.
